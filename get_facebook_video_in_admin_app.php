...
class LotteryNumbersClass
{
    ...
    public function getLotteryNumberList()
    {
        ...
        try {
            ...
            $count = $this->conn->prepare("SELECT COUNT(id) as total FROM facebook_video");
            $count->execute();
           ...
           $searchStm = $this->conn->prepare("SELECT facebook_video.*, COUNT(fb_video_views.id) AS views FROM `facebook_video` LEFT JOIN fb_video_views ON facebook_video.id = fb_video_views.videoId GROUP BY facebook_video.id ORDER BY facebook_video.id DESC LIMIT $startAt, $perPage");
