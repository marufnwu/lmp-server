...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $fourceUpdate = $_REQUEST["fourceUpdate"];
    $alertUpdate = $_REQUEST["alertUpdate"];

    $imageInfo = pathinfo($_FILES["image"]["name"]);
    $imageExtensionInfo = $imageInfo["extension"];
    $imageUrl = $uploadUrl . "images/ads_image_" . getFileName() . "." . $imageExtensionInfo;
    $imagePath = $uploadPath . "images/ads_image_" . getFileName() . "." . $imageExtensionInfo;

    try {
        move_uploaded_file($_FILES["image"]["tmp_name"], $imagePath);
        $insertStm = "UPDATE tbl_app_update SET version_code='$versionCode', video_thumbail='$imageUrl', video_link='$videoLink', description='$Description', update_position='$fourceUpdate', status='$alertUpdate' WHERE id='1'";
