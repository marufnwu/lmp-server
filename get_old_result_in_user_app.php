...
class LotteryNumbersClass
{
    ...
    public function getLotteryNumberList()
    {
        ...
        try {
            $searchStm = $this->conn->prepare("SELECT WinTime, WinDate, SlotId FROM lottery_number_table GROUP BY WinDate, WinTime ORDER BY STR_TO_DATE(WinDate,'%d-%m-%Y') DESC, WinTime");
            $searchStm->execute();
