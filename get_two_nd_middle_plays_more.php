...
class LotteryNumbersClass
{
    ...
    public function getLotteryNumberList()
    {
        ...
        try {
           ...
           $searchStm = $this->conn->prepare("SELECT *, COUNT(LotteryNumber) FROM lottery_number_table WHERE WinType='2nd' GROUP BY SUBSTR(LotteryNumber,2,2) HAVING (COUNT(LotteryNumber) > 1)  ORDER BY COUNT(LotteryNumber) DESC, SUBSTR(LotteryNumber,2,2) DESC");
