...
class ResultInfoClass{
    ...
    public function getResultInfos(){
        ...
        try {
            $searchStm = $this->conn->prepare("SELECT lottery_number_table.*, lottery_result_time.time FROM lottery_number_table INNER JOIN lottery_result_time ON lottery_number_table.SlotId =  lottery_result_time.id WHERE (SUBSTR(LotteryNumber,2,2)='$this->lotteryNumber' AND (LENGTH(LotteryNumber) > 4)) OR (SUBSTR(LotteryNumber,1,2)='$this->lotteryNumber' AND (LENGTH(LotteryNumber) < 5)) ORDER BY STR_TO_DATE(WinDate,'%d-%m-%Y') DESC, WinTime DESC");
            $searchStm->execute();
            ...
            if ($searchStm->rowCount()==0) {
                ...
                $searchResult = array();
                while($row = $searchStm->fetch(PDO::FETCH_ASSOC)){
