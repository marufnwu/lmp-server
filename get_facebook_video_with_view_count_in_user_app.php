...
class LotteryNumbersClass
{
    ...
    public function getLotteryNumberList()
    {
        ...
        try {
           ...
           $searchStm = $this->conn->prepare("SELECT facebook_video.id, facebook_video.video_title, facebook_video.thumbail, facebook_video.video_link, facebook_video.active, facebook_video.random_views as total_views FROM `facebook_video` WHERE active=1  ORDER BY facebook_video.id DESC LIMIT $startAt, $perPage");
