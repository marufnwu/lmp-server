...
class ResultInfoClass{
    ...
    public function getResultInfos(){
        ...
        try {
            $searchStm = $this->conn->prepare("SELECT * FROM tutorial_table ORDER BY Id DESC");
            $searchStm->execute();
            $searchResult = $searchStm->fetchAll(PDO::FETCH_ASSOC);
            if (count($searchResult) === 0) {
                $this->response["status"]="not exist";
                $this->response["message"]="No information found about this Tutorial image.";
                echo json_encode($this->response);
            } else if (count($searchResult) >= 1) {
                $this->response["status"]="success";
                $this->response["message"]="Tutorial image information found successfully.";
