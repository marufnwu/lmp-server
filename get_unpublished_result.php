...
class LotteryNumbersClass
{
    ...
    public function getLotteryNumberList()
    {
        ...
        try {
            $searchStm = $this->conn->prepare("SELECT *, COUNT(WinDate),COUNT(WinTime) FROM lottery_number_table GROUP BY LotterySerialNumber ORDER BY STR_TO_DATE(WinDate,'%d-%m-%Y') DESC, WinTime");
            $searchStm->execute();
            ...
            if (count($searchResult) === 0) {
                ...
                //echo json_encode($searchResult[0]);
                ...
                foreach ($searchResult as $value) {
                    ...
                    for($i = 1; $i<=3; $i++){
                        ...
                        foreach ($searchResult as $value) {
                            if($date == $searchResult['WinDate']){
                                if($searchResult['SlotId']=="$i"){
